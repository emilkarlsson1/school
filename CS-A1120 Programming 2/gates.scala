...
class OrGate(in1: Gate, in2: Gate) extends Gate() {
  ...
  def depth: Int = (in1.depth max in2.depth) +1
...
class XorGate(in1: Gate, in2: Gate) extends Gate() {
  ...
  def depth: Int = (in1.depth max in2.depth) +2 
  def support: Set[Gate] = Set(this) ++ in1.support ++ in2.support 
}
...
class MajorityGate(in1: Gate, in2: Gate, in3: Gate) extends Gate() {
  def value = {
    if (in1.value && in2.value || in1.value && in3.value || in2.value && in3.value) {
      true
    } else {
      false
    }
  ...
  def depth: Int = (in1.depth max in2.depth max in3.depth) +1
