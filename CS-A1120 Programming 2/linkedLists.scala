...
abstract class LinkedList[A]() {
  ...
  final def count(p: A => Boolean): Int = {
    def inner(list: LinkedList[A], counter: Int) : Int = {
        list match{
            case Nil() => counter
            case Cons(h,t) => if(p(h)){inner(t, counter+1)} else {inner(t, counter)}
        }    
      }
    inner(this, 0)
  }
  ...
  final def filterNot(p: A => Boolean): LinkedList[A] = {
    @tailrec def inner(remaining: LinkedList[A], result: LinkedList[A]): LinkedList[A] =
      remaining match {
        case Nil() => result
        case Cons(h, t) => inner(t, if(!p(h)) Cons(h, result) else result)
      }
  ...
  final def drop(n: Int): LinkedList[A] = {
    @tailrec def inner(remaining: LinkedList[A], count: Int): LinkedList[A] = {
      if(remaining.isEmpty){
        return Nil()
      }else{
        count match {
          case 0 => remaining
          case _ => inner(remaining.tail, count - 1)
        }
       }
      }
    inner(this, n)
  ...
  final def map[B](f: A => B): LinkedList[B] = {
    @tailrec def inner(remaining: LinkedList[A], result: LinkedList[B]): LinkedList[B] = 
      remaining match {
        case Nil() => result
  ...
  final def foldLeft[B](start: B)(f: (B, A) => B): B = {
    @tailrec def inner(remaining: LinkedList[A], result: B): B = 
      remaining match {
        case Nil() => result
  ...
  final def splitAt(n: Int): (LinkedList[A], LinkedList[A]) = {
    if (n <= 0) {
      ...
      (this.reverse.drop(this.length - n).reverse, this.drop(n))
    }
  }
