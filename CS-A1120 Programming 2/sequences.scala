...
package object sequences {
  ...
  def removeOdd(in: Seq[Int]): Seq[Int] = in.filter(_%2!=1)
  ...
  def sumOfSquaresOfOdd(in: Seq[Int]): Int = {
    val col = in.filter(_ % 2!=0)         // (1)  [when making a cascade it is convenient ..]
    val res = col.map(a=> a*a)            // (2)  [.. to break the cascade into multiple]
    res.reduce((a,b)=> a+b)         // (3)  [.. lines to ease readability]
  }
  /*
   * Task 5:
  ...
  def productOfNonzero(in: Seq[Int]): Int =
  {
    val col = in.filter(_ != 0)
    col.reduce((a,b)=> a*b)
  }
  ...
  {
    val col = in.filter(_> 0)
    val colu = col.map(a=> a*a)
    val res = colu.filter(_>= 17)
  ...
  {
    val yhd = (a zip b)
    val dif = yhd.map(x=> x._2 - x._1)
    val sqr = dif.map(a => a*a)
    sqr.sum
  }
  ...
  {
    val eka = a.zipWithIndex
    val tok = eka.maxBy(x => x._1)
    tok._2
  ...
  {
    val yh = a.zip(b)
    val plus = yh.map(x => x._1 + x._2)
    val miin = yh.map(x => x._1 - x._2)
    plus.zip(miin).unzip
  }
  ...
  def movingSum(in: Seq[Int], w: Int): Seq[Int] = {
    ...
    val ek = in.scan(0)(_+_)
    val eka = ek.drop(w)
    val toka = ek.dropRight(w)
    val kok = (eka zip toka)
    kok.map(x => x._1 -x._2)
