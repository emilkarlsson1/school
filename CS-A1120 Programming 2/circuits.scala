...
object factory {
  ...
  def buildEqual(bus1: Bus, bus2: Bus): Gate = {
    ...
    var yhdorg = (bus1 zip bus2)
    var yhdorg1 = yhdorg.map(i => i._1 && i._2)

    var negbus1 = ~bus1
    var negbus2 = ~bus2
    var yhdneg = (negbus1 zip negbus2)
    var yhdneg1 = yhdneg.map(i => i._1 && i._2)

    var out = (yhdneg1 zip yhdorg1)
    out.map(i => i._1 || i._2).reduceLeft( _ && _)
  ...
  def buildUnsignedLess(bus1: Bus, bus2: Bus): Gate = {
    ...
    for (i <- 0 until bus1.length) {
      out = buildGategi(bus1(i), bus2(i), out)
    }
    out
  }
   ...
   *
   */
  def buildUnsignedLessOrEqual(bus1: Bus, bus2: Bus): Gate = (buildEqual(bus1, bus2) || buildUnsignedLess(bus1, bus2))
  ...
  def buildIncrementer(bus: Bus): Bus = {
    ...
    var carry_in: Gate = Gate.True // no initial carry
  ...
  def buildCountTrueBits(bus: Bus): Bus = {
    ...
    var out = Bus.falses(log2ceil(bus.length))
    for (i <- 0 until bus.length) {
      out = buildAdderNoOverflow(out, new Bus(Array(bus(i))))
    }
    
    out.take(log2ceil(bus.length))
  }
  ...
  def buildMajorityTester(bus: Bus): Gate = {
    ...
    var negbus = ~bus
    var truebus = buildCountTrueBits(bus)
    var negbus1 = buildCountTrueBits(negbus)
    buildUnsignedLess(negbus1, truebus)
  ...
  def buildMultiplier(bus1: Bus, bus2: Bus): Bus = {
    ...
    var mult = bus2.map(x => x && bus1(0))
    var out = mult ++ Bus.falses(bus1.length)
    
    for (i <- 1 until bus1.length) {
      mult = bus2.map(x => x && bus1(i))
      out = buildAdder(out, Bus.falses(i) ++ mult ++ Bus.falses(bus1.length - i) )
    }
