...
object factory {
  ...
  def buildOneWordMemory(readEnable: Gate, writeEnable: Gate, data: Bus): Bus = {
    ...
    var x = readEnable.host.inputs(data.length)
    var selector = buildTwoBusSelector(x, data, writeEnable)
    var y = readEnable.host.falses(data.length)
    x.buildFeedback(selector)
    
    var out = buildTwoBusSelector(y, x, readEnable)
    out
  }
  ...
  def buildMemory(readEnable: Gate, writeEnable: Gate, address: Bus, data: Bus): Bus = {
    ...
    require(data.length > 0, "Data bus must have positive width.")
    var osoite = buildDecoder(address)
    var jono = Seq[Bus]()
    
    for (i <- 0 to osoite.length - 1) {
      var x = osoite(i) && writeEnable
      jono = jono :+ buildOneWordMemory(readEnable, x, data)
    }
    
    buildBusSelector(jono, osoite)
  }
}
