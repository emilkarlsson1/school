  ...
  final def expand: Expr = {
    def negate (x: Expr): Expr = 
      x match {
          case Var(n) => -x
          case Num(v) => Num(-v)
          case Multiply(l, r) => Multiply(negate(l), r)
          case Add(l, r) => Add(-l, -r)
          case Subtract(l, r) => Subtract(l, -r)
          case Negate(x) => x
      }
    
    def SOPList (x: Expr): List[Expr] =
      x match {
          case Var(n) => List(Var(n))
          case Num(v) => List(v)
          case Multiply(l, r) => SOPList(l).flatMap(y => SOPList(r).map(x => (x * y)))
          case Add(l, r) => SOPList(l) ++ SOPList(r)
          case Subtract(l, r) => SOPList(l) ++ SOPList(negate(r))
          case Negate(i) => SOPList(i).map(x => negate(x))
      }
    
    SOPList(this).reduceLeft(_ + _).simplify   
  }
}
