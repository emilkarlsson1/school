...
package object promises {
  ...
  def buildPlusF(): (Promise[Int], Promise[Int], Future[Int]) = {
      var eka = Promise[Int] 
      var toka = Promise[Int]
      var out = for(x <- eka.future; y <- toka.future) yield x + y
  ...
  def buildPlusZeroFailF(): (Promise[Int], Promise[Int], Future[Int]) = {
      ...
      var toka = Promise[Int]
      var out = for(x <- eka.future; y <- toka.future) yield x + y
      var out2 = out.fallbackTo((Future(0)))
  ...
  def fastestOfTwo(a: Future[Int], b: Future[Int]): Future[Int] = {
      var eka = Promise[Int] 
      var toka = Promise[Int]
      eka.tryCompleteWith(a).tryCompleteWith(b).future
  }
  ...
  def fastestSuccessfulOfTwo(a: Future[Int], b: Future[Int]): Future[Int] = {
    ...
    a.onSuccess{case x => eka.trySuccess(x)}
    b.onSuccess{case x => eka.trySuccess(x)}
    
    a.onFailure{case x => b.onFailure{case z => eka.tryFailure(z)} }
