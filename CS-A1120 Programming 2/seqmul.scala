...
object factory {
  ...
  def buildSequentialMultiplier(aa: Bus, bb: Bus, loadEnable: Gate): (Gate, Bus) = {
    var mA = loadEnable.host.inputs(aa.length + bb.length)
    var mB = loadEnable.host.inputs(aa.length + bb.length)
    var res = loadEnable.host.inputs(aa.length + bb.length)
    
    var x = loadEnable.host.falses(1) ++ mA.dropRight(1)
    var y = aa ++ loadEnable.host.falses(bb.length)
    var outA = (x && !loadEnable) | (y && loadEnable)
    mA.buildFeedback(outA)

    var x2 = mB.drop(1) ++ loadEnable.host.falses(1)
    var y2 = bb ++ loadEnable.host.falses(aa.length)
    var outB = (x2 && !loadEnable) | (y2 && loadEnable)
    mB.buildFeedback(outB)

    var resx = buildAdder(res, mA && mB(0))
    var resy = loadEnable.host.falses(aa.length + bb.length)
    var outres = (resx && !loadEnable) | (resy && loadEnable)
    res.buildFeedback(outres)       
    
    var finished = ((~mA).reduce(_ && _) || (~mB).reduce(_ && _)) && (!loadEnable)
