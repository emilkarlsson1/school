...
package object base64 {
  ...
  def restrictedEncode(b: Array[Byte]): String = {
    ...
    while (arr.hasNext) {
      var next = arr.next()
      var out = to6BitWords(to24Bits(next(0),next(1),next(2)))
      for (i <- 0 to 3) {
        lista += B64(out(i))
      }
    }
  ...
  def encode(b: Array[Byte]): String = {
    if (b.length % 3 == 0) {
      ...
      var ulos = restrictedEncode(b :+ 0.toByte :+ 0.toByte)
      ulos.dropRight(2) + "=="
    } else {
      var ulos = restrictedEncode(b :+ 0.toByte)
      ulos.dropRight(1) + "="
    }
  }
